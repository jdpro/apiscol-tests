...
public class TagNameTest extends ApiScolTests{
	...
	public void testTagName(){
		...
		assertTrue("The root name should be <entry>, not "+rootName,rootName.equals("entry"));
		NodeList nl=root.getChildNodes();
		assertTrue("There should be 11 childs under <entry> and here there are "+nl.getLength(),nl.getLength()==11);
		String[] tabNameNl={"updated","id","author","title","content","link","link","link","link","link","link"};
		for (int i=0;i<nl.getLength();i++){
			String name=nl.item(i).getNodeName();
			assertTrue("the node number "+(i+1)+" name should be <"+tabNameNl[i]+">, not "+name,name.equals(tabNameNl[i]));
		}
